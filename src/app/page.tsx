...
export default function Home() {
    ...
    const updateShowAnimation1 = globalStore((state:any) => state.updateShowAnimation1)

    const showAnimation2 = globalStore((state:any) => state.showAnimation2)
    const updateShowAnimation2 = globalStore((state:any) => state.updateShowAnimation2)

    const showVideo = globalStore((state:any) => state.showVideo)
    const updateShowVideo = globalStore((state:any) => state.updateShowVideo)

    const updateShowNavbarOrFooter = globalStore((state:any) => state.updateShowNavbarOrFooter)
  ...
  return (
      <div className="w-full" style={{backgroundColor: navbarBackgroundColor}}>
          {loading ?
            <div className="flex justify-center items-center h-[100vh] w-full absolute top-0 left-0 bg-black"
                style={{backgroundColor: navbarBackgroundColor}}
            >
                {(!showAnimation1 && !showAnimation2) && <span className="loading loading-bars loading-lg text-white"/>}
                {showAnimation1 &&
              ...
              <>
                  {showVideo ? <div className="flex justify-center w-full h-[100vh]">
                      ...
                      <div className="flex flex-col items-center gap-2 absolute bottom-5 p-2 hover:cursor-pointer hover:border hover:border-white hover:rounded">
                          <span className="text-white z-50 shadow-xl text-xl font-medium"
                                onClick={() => {
                                    updateShowVideo(false)
                                    updateShowNavbarOrFooter(true)
                                    updateNavbarBackgroundColor("#04203b")
